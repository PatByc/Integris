...
export function InvoiceForm({ documentId, initialDraft, onSaved }: Props) {
  ...
  return (
    <div className="space-y-6">
      <section>
        <h3 className="mb-3 text-sm font-semibold text-gray-700">Seller</h3>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Name" value={form.seller_name} onChange={set("seller_name")} />
          <Field label="NIP" value={form.seller_nip} onChange={set("seller_nip")} />
          <div className="col-span-2">
            <Field label="Address" value={form.seller_address} onChange={set("seller_address")} />
          </div>
        </div>
      </section>

      <section>
        <h3 className="mb-3 text-sm font-semibold text-gray-700">Buyer</h3>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Name" value={form.buyer_name} onChange={set("buyer_name")} />
          <Field label="NIP" value={form.buyer_nip} onChange={set("buyer_nip")} />
          <div className="col-span-2">
            <Field label="Address" value={form.buyer_address} onChange={set("buyer_address")} />
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Sale date" value={form.sale_date} onChange={set("sale_date")} type="date" />
          <Field label="Payment due" value={form.payment_due_date} onChange={set("payment_due_date")} type="date" />
          <Field label="Payment method" value={form.payment_method} onChange={set("payment_method")} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Field label="Net total" value={form.net_total} onChange={set("net_total")} />
          <Field label="VAT total" value={form.vat_total} onChange={set("vat_total")} />
          <Field label="Gross total" value={form.gross_total} onChange={set("gross_total")} />
        </div>
      </section>
